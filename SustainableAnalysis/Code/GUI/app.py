...
def search():
	...
	query1 = "SELECT Title, Date, Author, Text, Totalviews FROM article;"
	if not info:
		rows = connect(query1 + "")
		return render_template('selectresult3.html', rows=rows)
	else:
		rows = connect(query + "")
		if rows is "":
...
def searchUser():
	...
	query = "SELECT userid, name, username, permissionset FROM users WHERE username ILIKE \'" + usersearchid + "\';"
	print(query)
	rows = connect(query + "")
	if rows is "":
		return render_template('failed.html')
